...
def _append_value(session, mapobject_id, key, val):
...
#        )
    ...
    connection.execute('''SELECT master_modify_multiple_shards('UPDATE feature_values SET values = values || hstore(''key'',''val'') WHERE mapobject_id = mapobject_id');''', {'mapobject_id':mapobject_id,
    'key': key,
    'val': val}
)
