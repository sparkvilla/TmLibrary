...
from tmlib.workflow.dependencies import get_workflow_definition
...
class WorkflowDescription(object):
    ...
    workflow If a dependency is not available, i.e. if a required input
    of a step has not been generated by another upstream step,
    the workflow will stop.
    However, in order to be able to check the workflow logic before submitting
    jobs, workflows dependencies are checked using an instance of
    ...
    def __init__(self, type, stages=[]):
        ...
        Note
        ----
        When no `stages` are provided a description with default values
            ...
            for step in steps:
                ...
                self.add_step(
                    WorkflowStepDescription(
                        name=step['name'], active=step['active'], ordinal,
