...
class ImageAnalysisPipelineEngine(WorkflowStepAPI):
    ...
    def delete_previous_job_output(self):
        ...
        with tm.utils.ExperimentSession(self.experiment_id, False) as session:
            if self.ordinal == 1:
                # If it's the first pipeline all non-static objects can be
                # deleted to also clean up potential dependendies of
                # downstream pipelines.
                mapobject_types = session.query(tm.MapobjectType.id).\
                    filter(tm.MapobjectType.ref_type == None).\
                    one()
            else:
                # Only delete mapobject types that are defined as output to
                # not delete mapobjects generated by other pipelines.
                names = [
                    obj.name
                    for obj in self.project.pipe.description.output.objects
                ]
                mapobject_types = session.query(tm.MapobjectType.id).\
                    filter(tm.MapobjectType.name.in_(names)).\
                    all()
            for mapobject_type in mapobject_types:
                logger.info('delete mapobject type "%s"', name)
                session.query(tm.Mapobject).\
                    filter_by(mapobject_type_id=mapobject_type.id).\
                    delete()
                session.query(tm.MapobjectType).\
                    filter_by(id=mapobject_type.id).\
                    delete()

    def _load_pipeline_input(self, roi_id, well_id):
        ...
        with tm.utils.ExperimentSession(self.experiment_id) as session:
            roi_segment = session.query(tm.MapobjectSegmentation).\
                filter_by(mapobject_id == roi_id, partition_key == well_id).\
                one()
            ...
            site_mapobjects = session.query(tm.Mapobject).\
                ...
                filter(
                    tm.MapobjectSegmentation.geom_polygon.ST_Intersect(
                        roi_segment.geom_polygon.ST_Envelope()
                    ),
                    tm.MapobjectSegmentation.partition_key == well_id,
            ...
            # TODO: do this for each channel separately
            ...
            for ch in channel_input:
                ...
                if channel.bit_depth == 16:
                    ...
                    dtype = np.uint8
                store['pipe'][ch.name] = np.zeros(
                    (roi_height, roi_width, n_zplanes, n_tpoints), dtype
                )
                if ch.correct:
                    logger.info(
                        'load illumination statistics for channel %d',
                        channel_id
                    )
                    try:
                    ...
                    illumstats[channel.id] = stats_file.get()
                else:
            ...
            for sid in site_ids:
                ...
                for ch in channel_input:
                    ...
                    for f in image_files:
                        ...
                        img = f.get()
                        if ch.correct:
                            logger.info('correct channel image %d', f.id)
                            img = img.correct(illumstats[ch.id])
    ...
    def _save_pipeline_outputs(self, store, assume_clean_state):
        ...
        with tm.utils.ExperimentSession(self.experiment_id, False) as session:
            ...
            roi_mapobject_type = session.query(tm.MapobjectType).\
                filter_by(name=self.project.pipe.description.input.roi).\
                one()
                    ...
                    for t, z, label, polygon in iterator:
                        ...
                        mapobject_segmentations.append(
                            tm.MapobjectSegmentation(
                                partition_key=store['well_id'], label=label,
