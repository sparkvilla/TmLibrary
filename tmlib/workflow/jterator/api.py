...
class ImageAnalysisPipelineEngine(WorkflowStepAPI):
    ...
    def delete_previous_job_output(self):
        ...
        # Only delete mapobject types that are defined as output of the pipeline,
        # since there might be multiple consecutive pipelines.
        # This might lead to problems, however, if users modify the pipeline
        # in between runs, i.e. first define a mapobject type in the output
        # section of the pipeline and then remove it afterwards. In this case
        # the old data will no longer be cleaned up.
        with tm.utils.ExperimentSession(self.experiment_id, False) as session:
            for obj in self.project.pipe.description.output.objects:
                name = obj.name
                logger.info('delete mapobjects of type "%s"', name)
